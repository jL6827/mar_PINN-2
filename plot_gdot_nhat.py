...
# plot_gdot_nhat.py
# 说明：
#   计算并绘制重力加速度向量 g 与温跃层界面法向单位向量 n̂ 的点积的空间分布图。
#
# 物理/数学原理（含符号约定）：
...
#       n̂ = n / ‖n‖，  ‖n‖ = sqrt((∂Z0/∂x)² + (∂Z0/∂y)² + 1)
#
#   重力加速度向量（竖直向下为负 z 方向）：
#       g = (0, 0, -g0)，  g0 ≈ 9.81 m/s²
#
#   点积（即重力在法向上的分量）：
#       g · n̂ = (0·n̂_x + 0·n̂_y + (-g0)·n̂_z)
#             = -g0 · (1 / ‖n‖)
#             = -g0 / sqrt(1 + (∂Z0/∂x)² + (∂Z0/∂y)²)
#
#   结果永远为负（约 [-9.81, 0)）：
...
def main():
    p = argparse.ArgumentParser(
        description=(
            "绘制重力加速度 g 与温跃层界面法向单位向量 n̂ 的点积 g·n̂ 的空间分布图。\n"
            "物理意义：g·n̂ 反映重力加速度在界面法方向上的分量（'法向重力分量'），\n"
    ...
    #     - 归一化为单位向量 n̂ = n / ‖n‖
    #     - 点积 g · n̂ = (0,0,-g0) · n̂ = -g0·n̂_z = -g0 / ‖n‖
